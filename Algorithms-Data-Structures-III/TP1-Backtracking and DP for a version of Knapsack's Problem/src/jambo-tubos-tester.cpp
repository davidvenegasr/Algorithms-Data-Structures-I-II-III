...
int BT(int i, int cant_hasta, int min_resist)
{
    ...
    if (poda_optimalidad){
		bool p_resistencia = cant_hasta + 1 + min_resist <= MS;
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < n; ++i) cin >> items_weight[i];
    for (int i = 0; i < n; ++i) cin >>  items_resistance[i];*/

    ////TESTER---------------------------------------------------
    //ifstream fin("/home/luis/CLionProjects/Algo3-TP1/input.txt");
    ifstream fin("input.txt");
    cout << "Archivo leido: " << (fin.is_open() ? "Bien" : "Mal") << endl;
    int buenas = 0;
    int malas = 0;
    while (!fin.eof()) {
        char separador;
        int res;
        int nroTest;
        for (int i = 0; i < 10; ++i) fin >> separador;
        fin >> nroTest;
        fin >> n >> R_tubo;
        items_weight.assign(n, 0);
        items_resistance.assign(n, 0);
        for (int i = 0; i < n; ++i) fin >> items_weight[i];
        for (int i = 0; i < n; ++i) fin >> items_resistance[i];
        fin >> res;
        ////TESTER---------------------------------------------------
        ...
        //TESTER------------------------------------------------
        res == optimum ? buenas++ : malas++;
        cout <<"Test nro " <<nroTest <<(res == optimum ? ": Correcto" : ": Mal") <<endl;
        cout <<"Esperado " << res << " VS " << optimum <<endl;
        cout <<"------------------------------"<<endl;
        //TESTER---------------------------------------------------
    }
    cout <<"TestPasados"<<buenas<<'/'<< buenas+malas<<endl;
    return 0;
}
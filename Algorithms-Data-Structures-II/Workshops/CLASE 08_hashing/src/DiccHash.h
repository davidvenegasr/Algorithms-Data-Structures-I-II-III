...
public:
    ...
    // Devuelve true si la clave está definida en el diccionario. ~ clav : clave a buscar
    bool definido(const string& clav) const;

   /* Agrega una clave y su significado al Diccionario.
    * Si la clave a agregar ya se encuentra definida, redefine su significado.
    * - clav : clave a agregar - sig  : significado de la clave a agregar
    *
...
private:
    ...
    struct Asociacion {
            string clave;
            V valor;
            bool operator==(const DiccHash::Asociacion &h1) const;
    };

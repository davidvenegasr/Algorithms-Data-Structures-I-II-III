...
template <typename T>
bool string_map<T>::operator==(const string_map<T>& d) {
    bool a = igualarNodos(_raiz, d._raiz);
    return (_size == d._size) && a;
}
...
template<typename T>
set<string> string_map<T>::claves(){
    return _claves;
}
